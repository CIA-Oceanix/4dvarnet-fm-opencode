...
def write_report(exp_root: Path, output_path: Path) -> None:
    ...
    md.append("- **MonaiDirectUNet (L1b + MONAI's DiffusionModelUNet backbone): normalization "
              ...
              "Training loss descended smoothly the whole run (0.75 -> 0.017 train, val "
              "plateauing ~0.114-0.117 from epoch ~50) with no flatlining. Final RMSE "
              "0.539/0.541 (S0/S1) -- *better* than L1b's own unnormalized Original (0.62/0.63) "
              "and ~3.2x better than L1b's collapsed normalized Phase 2 (1.73/1.72). EV "
              "0.885/0.884, on par with VanillaCFM's normalized result (0.884/0.883) despite "
              "being architecturally a single-pass deterministic regressor like DirectUNet, not "
              "a flow-matching model with an inherent noise-scale signal. This is consistent "
              "with the hypothesis that DirectUNet's collapse under normalization was a "
              ...
              "normalized input distribution just as well as (here, slightly better than) a "
