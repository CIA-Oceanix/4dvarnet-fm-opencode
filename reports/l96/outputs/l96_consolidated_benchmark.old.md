> **SUPERSEDED — frozen 2026-09-18.** This is the consolidated benchmark as it
> stood before the reproducibility refactor. It is kept as a record of the
> retired rows and of the earlier metric conventions; it is **not** regenerated
> and must not be cited for current numbers. Use
> `l96_consolidated_benchmark.md`.
>
> What changed:
> - **16 pre-monai neural methods retired** (`L1b_direct_unet_s0s1`, `L2b`/`L3`/
>   `L4`/`L5`/`L6`, `V2_tweedie_cfm_l96`, `V3_predict_state_cfm_l96`,
>   `SDA1_prior_l96`, `SDA2_cond_*`, `FDV1_unrolled_unet_l96`,
>   `FDV1CFM_predict_state_l96`, `FDV1_SDA*_hybrid_l96`) — superseded by their
>   monai-backbone successors. Their numbers live on only in this file.
> - **Seven methods here had no generator path at all** and were hand-written
>   into the markdown by PRs #188/#192/#194/#200/#204; they are now produced by
>   `generate_l96_consolidated_report.py` from the stored arrays.
> - **ES/CRPS mixed two formulas** — a proper N=30 ensemble score and an N=1 MAE
>   proxy — separated only by a trailing `*`. The proxy has been removed.
> - `DirectUNet-M(monai,flat)`'s slow/obs_fast split here came from a
>   `neural_eval.json` sidecar that disagrees with its own stored arrays; the
>   current report recomputes it.
