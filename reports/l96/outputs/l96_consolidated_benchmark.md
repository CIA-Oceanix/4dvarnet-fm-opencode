...
**Note on the three `obsdensity`/`(aug)` rows:** `DirectUNet-M(monai,cos,obsdensity)`, `CFM-M(monai,flat,obsdensity)`, and `DirectUNet(aug)+SDA3` were added by directly computing `evaluation/estimate_metrics.py`'s pooled and per-window functions against their `estimates_{s0,s1}[_keep16].npz` (same convention as every other row here), rather than by rerunning this script's full regeneration path -- that path also needs the DA-baseline/joint-comparison cache files, which live only in the `experiments/` directory of the worktree these were trained/evaluated in, not this one. The two plain mean-model rows are genuinely single-pass (N=1); `DirectUNet(aug)+SDA3` is a 30-member-ensemble hybrid at eval time (`ens30×10`, same convention as the non-augmented `DirectUNet+SDA3`/`FDV1+SDA3` rows) but its stored `.npz` only kept the per-repeat ensemble **mean** trajectory (`eval_obs_density_l96.py`'s sweep convention), not the raw members -- so unlike its non-augmented sibling rows, its ES/CRPS here still fall back to the N=1 MAE proxy (`*` in the ES/CRPS tables), not a true ensemble spread score; RMSE/EV are unaffected by this (mean-based either way). All three are full canonical density only (`keep16`); see `l96_obs_density_augmented_training.md` for their dedicated reduced-density sweep (the actual point of the `obs_density_augment` training feature).

**Note on the two `FDV1-Stier` rows:** `FDV1-Stier(monai)` and `FDV1-Stier+SDA3(monai)` were likewise added by computing `evaluation/estimate_metrics.py`'s pooled and per-window functions directly against saved `estimates_{s0,s1}.npz`/`members_{s0,s1}.npz` (same convention as the rows above), not via this script's full regeneration path, for the same DA-baseline/joint-comparison-cache reason (those caches live only in the `experiments/` directory of the worktree these were trained/evaluated in). `FDV1-Stier(monai)` is a genuine single-pass (N=1) mean estimate; `FDV1-Stier+SDA3(monai)` is a true 30-member ensemble hybrid with its raw members retained, so its ES/CRPS entries are proper ensemble scores (no `*`), unlike the `DirectUNet(aug)+SDA3` row above. One cross-worktree gotcha worth flagging here: evaluating the SDA3 hybrid from the `4dvarnet-fm-fdv-monai` worktree (needed for its checkpoint/norm-stats/cached test dataset) silently builds the wrong UNet tier, because that worktree predates this session's `monai_num_res_blocks` feature -- see the `FDV1-Stier+SDA3(monai)` row's own description below for the full story and the fix (rerun from `4dvarnet-fm-obs-density-gen` instead, with absolute paths into the other worktree's `experiments/` dir).

**Note on the two `subgrad+state-Stier` rows:** `subgrad+state-Stier(monai)` and `subgrad+state-Stier+SDA3(monai)` use the identical direct-`evaluation/estimate_metrics.py`-against-npz methodology as the two `FDV1-Stier` rows above (not this script's full regeneration path, for the same DA-baseline/joint-comparison-cache reason). `subgrad+state-Stier(monai)` is a genuine single-pass (N=1) mean estimate, evaluated straight from `experiments/FDV2_subgrad_state_monai_l96_Stier/estimates_{s0,s1}.npz` in the `4dvarnet-fm-obs-density-gen` worktree; `subgrad+state-Stier+SDA3(monai)` is a true 30-member ensemble hybrid with its raw members retained, so its ES/CRPS entries are proper ensemble scores (no `*`), same convention as `FDV1-Stier+SDA3(monai)`.

**Note on the two `grad+state-Stier-priorresidual` rows:** same direct-`evaluation/estimate_metrics.py`-against-npz methodology as every row above, evaluated straight from `experiments/FDV2_grad_state_monai_l96_Stier_priorresidual_{N5,tbptt2x5}/estimates_{s0,s1}.npz`. Both are the REAL `update_input=grad+state` gradient-conditioned solver (`torch.autograd.grad(var_cost, x, create_graph=True)`, unlike `subgrad+state`'s cheap proxy above), made trainable at this S-tier via two new `FourDVarNetSolver` fixes -- `prior_residual` and `prior_dropout` -- motivated by a Jacobian decomposition of an earlier plateaued checkpoint (see the rows' own descriptions below for the full story). Both are genuine single-pass (N=1) mean estimates, same convention as `subgrad+state-Stier(monai)`. Neither is a new best on any metric -- included as a documented negative/informative result: the fixes eliminate the divergence that killed every earlier non-`prior_residual` real-gradient attempt at this tier, but the real gradient still trains to a meaningfully worse reconstruction than `subgrad+state`'s cheap proxy.
...
| grad+state-Stier-priorresidual-N5(monai) | Neural (4DVarNet-style unrolled solver, real-gradient-conditioned, monai backbone, S-tier) | FDV2's `update_input=grad+state` solver -- a single combined `torch.autograd.grad(var_cost, x, create_graph=True)` of `prior_weight*prior_cost(x) + obs_cost(x,obs)`, unlike `subgrad+state`'s cheap proxy above -- at the same S-tier MonaiUNet1D as `FDV1-Stier(monai)`/`subgrad+state-Stier(monai)` and the same `init_state_var=0.1`/`aux_var_cost_weight=0.01` recipe, with `trainable_prior_weight=true` (genuinely matters here, unlike `subgrad+state` where it's provably vestigial due to separate-channel normalization cancellation -- `grad+state` sums `prior_weight*grad(prior_cost)` with `grad(obs_cost)` BEFORE normalizing). Two new fixes, motivated by a Jacobian decomposition of an earlier plateaued checkpoint (found the real `prior_cost` gradient's Jacobian term `-2*J^T@r`, `J=d(Phi)/dx`, dominating the useful residual term `2*r` by 11-53x in RMS and nearly orthogonal to it, cos~0.04-0.13 -- checked directly against installed `monai==1.6.0`: MonaiUNet1D already uses the same `zero_module()` zero-init convention as OpenAI's guided-diffusion UNet used successfully in `ocean4dvarnet`/`4dvarnet-global-mapping`'s `ronan_devs` branch, so that isn't the gap; `Phi(x)=prior_unet(x,tau)` here has no outer residual/skip wrapper, so zero-init only anchors it near identity AT INITIALIZATION, not throughout training): `prior_residual=true` (new `FourDVarNetSolver` param -- `Phi(x) = x + prior_unet(x, tau)`, a permanent identity anchor) and `prior_dropout=0.0` (new param, decouples `prior_unet`'s dropout from the main solver unet's `dropout=0.1` -- `prior_unet`'s forward is repeatedly re-run inside the `create_graph=True` double-backward, so dropout there would add mask noise to that computation on top of whatever the double-backward itself contributes; the main solver unet's own forward is always a single ordinary backward, so it can carry dropout freely). `N_outer=5` (half the depth of every other FDV1/FDV2 row in this table, a single truncated-BPTT block, chosen to shorten the double-backward chain and speed up iteration); 400 epochs, cosine-annealed LR. Both fixes eliminate the outright divergence/NaN that killed every earlier non-`prior_residual` `grad+state` attempt at this tier (including `FDV2(monai)` below) -- trains cleanly to a stable convergence (val_loss plateaued ~0.20, best 0.1998 @ epoch 391/400) -- but the real double-backward gradient still doesn't match `subgrad+state-Stier(monai)`'s quality, worse on every RMSE/EV/ES metric here; see `grad+state-Stier-priorresidual-tbptt2x5(monai)` below, which narrows but doesn't close that gap with a fuller unroll. Evaluated as a single pass (N=1), same convention as `subgrad+state-Stier(monai)`. |
| grad+state-Stier-priorresidual-tbptt2x5(monai) | Neural (4DVarNet-style unrolled solver, real-gradient-conditioned, monai backbone, S-tier) | As `grad+state-Stier-priorresidual-N5(monai)` above, but with the full `N_outer=10` depth restored (matching every other FDV1/FDV2 row), split into `tbptt_n_blocks=2` truncated-BPTT blocks of `tbptt_block_size=5` each (`FourDVarNetSolver`'s existing truncated-BPTT support -- the unrolled graph is detached at k=5) -- isolates the double-backward-chain-length effect from the iteration-count effect (does a shorter double-backward chain train better, independent of whether the model needs fewer iterations to converge). Meaningfully better than the single-block N5 variant on every metric here (~4-5% lower RMSE: 0.4098 vs 0.4286 pooled S0) -- the fuller unroll helps -- but still ~10% worse in pooled RMSE than `subgrad+state-Stier(monai)`'s cheap proxy. Its own training showed a slow, bumpy convergence (repeated lull-then-descend cycles rather than a clean monotonic one) and had not fully plateaued at the `stage1_best.ckpt` evaluated here (~140 of its 400 budgeted epochs remained, val_loss still very slowly improving, best 0.1867 at the time). Evaluated as a single pass (N=1), same convention as the N5 variant above. |
...
| DirectUNet(aug)+SDA3 | 0.3889 | 0.1818 | 0.4925 | 0.3894 | 0.1805 | 0.4939 | 1.001 |
| FDV1(monai) | 0.4251 | 0.2111 | 0.5320 | 0.4218 | 0.2093 | 0.5280 | 0.992 |
| FDV1-Stier(monai) | 0.4011 | 0.2042 | 0.4996 | 0.3993 | 0.2020 | 0.4980 | 0.996 |
| subgrad+state-Stier(monai) | 0.3728 | 0.1821 | 0.4681 | 0.3729 | 0.1801 | 0.4693 | 1.000 |
| grad+state-Stier-priorresidual-N5(monai) | 0.4286 | 0.2220 | 0.5318 | 0.4230 | 0.2198 | 0.5246 | 0.987 |
| grad+state-Stier-priorresidual-tbptt2x5(monai) | 0.4098 | 0.2219 | 0.5038 | 0.4076 | 0.2190 | 0.5020 | 0.995 |
| FDV2(monai) |   —   |   —   |   —   |   —   |   —   |   —   | n/a |
| FDV1+SDA3(monai) | 0.3787 | 0.1619 | 0.4871 | 0.3740 | 0.1580 | 0.4820 | 0.988 |
| FDV1-Stier+SDA3(monai) | 0.3587 | 0.1558 | 0.4601 | 0.3583 | 0.1524 | 0.4613 | 0.999 |
| subgrad+state-Stier+SDA3(monai) | **0.3410** | **0.1454** | **0.4388** | **0.3402** | **0.1438** | **0.4384** | 0.997 |
...
| DirectUNet(aug)+SDA3 | 0.9387 | 0.9911 | 0.9125 | 0.9381 | 0.9910 | 0.9117 |
| FDV1(monai) | 0.9279 | 0.9880 | 0.8979 | 0.9287 | 0.9879 | 0.8990 |
| FDV1-Stier(monai) | 0.9363 | 0.9888 | 0.9100 | 0.9364 | 0.9888 | 0.9102 |
| subgrad+state-Stier(monai) | 0.9443 | 0.9911 | 0.9210 | 0.9439 | 0.9911 | 0.9202 |
| grad+state-Stier-priorresidual-N5(monai) | 0.9276 | 0.9868 | 0.8980 | 0.9291 | 0.9867 | 0.9003 |
| grad+state-Stier-priorresidual-tbptt2x5(monai) | 0.9343 | 0.9866 | 0.9081 | 0.9345 | 0.9866 | 0.9085 |
| FDV2(monai) |   —   |   —   |   —   |   —   |   —   |   —   |
| FDV1+SDA3(monai) | 0.9406 | 0.9930 | 0.9144 | 0.9416 | 0.9931 | 0.9159 |
| FDV1-Stier+SDA3(monai) | 0.9469 | 0.9935 | 0.9237 | 0.9465 | 0.9936 | 0.9229 |
...
| FDV1-Stier(monai) | 0.2464* | 0.1540* | 0.2925* | 0.2451* | 0.1512* | 0.2920* |
| subgrad+state-Stier(monai) | 0.2226* | 0.1361* | 0.2659* | 0.2223* | 0.1338* | 0.2665* |
| grad+state-Stier-priorresidual-N5(monai) | 0.2653* | 0.1670* | 0.3145* | 0.2634* | 0.1646* | 0.3127* |
| grad+state-Stier-priorresidual-tbptt2x5(monai) | 0.2517* | 0.1700* | 0.2925* | 0.2510* | 0.1671* | 0.2930* |
| FDV2(monai) |   —   |   —   |   —   |   —   |   —   |   —   |
...
| grad+state-Stier-priorresidual-N5(monai) | 0.410±0.078 | 0.217±0.037 | 0.507±0.105 | 0.406±0.077 | 0.214±0.039 | 0.502±0.102 |
| grad+state-Stier-priorresidual-tbptt2x5(monai) | 0.390±0.078 | 0.217±0.037 | 0.477±0.105 | 0.389±0.078 | 0.214±0.037 | 0.476±0.104 |
| FDV2(monai) |   —   |   —   |   —   |   —   |   —   |   —   |
...
| grad+state-Stier-priorresidual-N5(monai) | 0.928±0.030 | 0.987±0.005 | 0.898±0.044 | 0.929±0.030 | 0.987±0.006 | 0.900±0.043 |
| grad+state-Stier-priorresidual-tbptt2x5(monai) | 0.934±0.030 | 0.987±0.006 | 0.908±0.043 | 0.934±0.029 | 0.987±0.006 | 0.908±0.042 |
| FDV2(monai) |   —   |   —   |   —   |   —   |   —   |   —   |
...
| grad+state-Stier-priorresidual-N5(monai) | 0.265±0.045* | 0.167±0.029* | 0.314±0.058* | 0.263±0.045* | 0.165±0.031* | 0.313±0.056* |
| grad+state-Stier-priorresidual-tbptt2x5(monai) | 0.252±0.044* | 0.170±0.029* | 0.293±0.056* | 0.251±0.044* | 0.167±0.029* | 0.293±0.056* |
| FDV2(monai) |   —   |   —   |   —   |   —   |   —   |   —   |
