#!/usr/bin/env python3
"""L96 joint state-parameter DA benchmark report builder (ETKF).
...
per case and group, per-param RMSE (+NRMSE) for the joint ETKF, and context
anchors against the L9 joint neural baseline (ens30x10). Missing values render
as ``--`` without crashing.
"""
import argparse
import json
...
def write_report(exp_dir: Path, json_path: Path, output_path: Path) -> None:
    ...
    md.append("# L96 Joint State-Parameter DA Benchmark (ETKF)")
    md.append("")
    ...
    md.append("")
    md.append("**Models:** Joint-ETKF estimates the 24D observed state **and** the 8 model "
              "parameters `F, c1, hx, eps, w1..w4` (fast weights per index; h fixed) via an "
              "augmented-state ensemble, benchmarked against the state-only vanilla ETKF on "
              "the same cached S0/S1 test set. On S1 (reduced 24D J=2 dynamics) the DA only "
              "carries `w1,w2`; `w3,w4` default to the reference prior `[1.0, 0.1]` and are "
              "marked with a `†` (not estimated).")
    md.append("")
    md.append("---")
    md.append("")
    ...
    if data is None:
        ...
        for m in methods:
            is_joint = "Joint" in m
            md.append(f"| {m} | {'Joint ETKF' if is_joint else 'Vanilla ETKF'} | "
                      f"{'yes' if is_joint else 'no (state only)'} |")
        md.append("")
        md.append("---")
        md.append("")
    ...
    md.append("The comparator loads the cached `l96_datasets_obsj2_int100_nwin200.pt` and runs "
              ...
              "param RMSE compares the joint ETKF's 8-wide estimate against `true_*` "
...
def main():
    parser = argparse.ArgumentParser(description="Generate the L96 joint DA (ETKF) benchmark report")
