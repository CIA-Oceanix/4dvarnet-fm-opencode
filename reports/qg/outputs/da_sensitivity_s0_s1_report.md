...
Data: `reports/qg/outputs/qg_obs_density_sweep/*.json` (N=10, 45 files: the original ETKF/EnKF density sweep, the cols=4/random sanity check, the EnKF cross-check, the inflation and `loc_radius` sweeps at cols=16/64, and the psi1+psi2 `loc_radius` sweep). Tests: `tests/test_qg_psi2_points.py` (15), `tests/test_qg_cols_sampling.py` (10) -- mechanical correctness only (shapes, determinism, no-hang, H-function/localization correctness), no scientific claim baked in. Scratch driver (not committed): `qg_obs_density_sweep_scratch.py`.
