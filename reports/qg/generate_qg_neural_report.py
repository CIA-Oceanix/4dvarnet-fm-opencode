...
def main() -> None:
    ...
    add("3 example test windows (best/median/worst by ETKF's per-window "
        ...
        "layers). Generated by `generate_qg_reconstruction_figs.py`.")
    add("")
    fig_dir = out_root / "figs"
    for label in ("best", "median", "worst"):
        fig_path = fig_dir / f"qg_s0_reconstruction_{label}.png"
        rel = fig_path.relative_to(out_root.parent.parent) if fig_path.exists() else None
        if rel is not None:
            add(f"### {label.capitalize()} window")
            add("")
            add(f"![{label} window reconstruction]({rel.as_posix()})")
            add("")
        else:
            add(f"### {label.capitalize()} window")
            add("")
