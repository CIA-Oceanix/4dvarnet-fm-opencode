...
- **Scenarios** `_scenario_window`:
  ...
  - **S1b** (`test_s1b`): **structural model error** — reduced-gravity single-layer `da_model="qg1l"` (not yet wired into `QGDynamics`; metadata + corrupted wind stored), params unbiased (= true draws).
...
- **To do (next)**: wire S1b `qg1l` reduced-gravity dynamics; QG DA baselines (EnKF/ETKF/4DVar on ψ/q) on the S0/S1 with held-out truth; then `train.py` dispatch + neural training on psi/q; parameter randomization as the final S0/S1 analog step.
