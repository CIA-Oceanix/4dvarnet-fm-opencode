...
- [x] `_lagged_init_ensemble()` functions for x(t₀-progress dt) members with dt~U(0,DT]
 ...
 - [x] **Dispersion over-scaling fix (PR #110)**: dispersion re-scaled to the **raw lagged per-point spread** (`init_ensemble.std(0).mean()`, bg-error-scaled) instead of the climatological state std — the latter over-dispersed ~40× once PR #109 cut the lag to 0.5d (bg error 5.5e-9 vs state std 2.3e-7), making DA worse than the free forecast. `spread_t0_mean` now reports per-point spread. Small-scale S0 (nx=16, 30d, lag 0.5): q-obs EV +1.000/psi-obs +1.000, improv 0.56/0.36 (was 0.02).
 - [x] **Lag-consistent free-forecast ref (PR #112)**: `_free_forecast_init` samples a `t₀−U(0,init_lag_days)` state from the buffer (end-relative) so `forecast_improvement` compares DA against the same-lag first guess; falls back to `init_state` for None. Otherwise the reference was pinned to 0.5d regardless of the ensemble lag.
 - [x] **Init-consistency fix + S0/30-day DA (PR #109)**: (a) `_lagged_init_ensemble` `dt_steps = int(init_lag_days * round(86400/cfg.dt))` (fixes the days/seconds unit bug `int(init_lag_days/cfg.dt)` that collapsed lag sampling to one step) and samples the buffer **end-relative** (`truth[len-k]`) so members are at t₀−U(0,init_lag_days] (not the buffer start ~t₀−10d); (b) `_free_forecast_rmse` rolls `window["init_state"]` (t₀−U(0,cfg.init_lag_days)) instead of `init_lead_truth[0]` (t₀−10d); (c) `init_lag_days` 2.0→0.5; (d) S0-only, `window_days=30` for q-obs density + psi-obs. Small-scale S0: q-obs EV +0.991, psi-obs EV +0.994, `mean_init_lag≈0.333`.
