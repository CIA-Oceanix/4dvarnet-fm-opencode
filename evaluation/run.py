...
# Joint (state + parameter) baseline counterparts -- each estimates
# sigma/rho/beta/c1 alongside the state, starting from the dataset's
# (possibly biased) per-window values as prior and reporting RMSE against
# the window's true_sigma/true_rho/true_beta/true_c1.
_JOINT_BASELINE_METHODS = ["Joint-Weak-4DVar", "Joint-Strong-4DVar", "Joint-EnKF", "Joint-ETKF"]
_JOINT_BASELINE_CLASSES = {
    "Joint-Weak-4DVar": JointWeak4DVar,
    "Joint-Strong-4DVar": JointStrong4DVar,
    "Joint-EnKF": JointEnKF,
    "Joint-ETKF": JointETKF,
}
_PARAM_NAMES = ("sigma", "rho", "beta", "c1")


def _baseline_traj_path(case_name, method_name, suffix=""):
    key = f"{case_name}_{method_name.replace('-', '_').replace(' ', '_')}"
    return os.path.join(BASELINES_DIR, f"baselines_trajs{suffix}_{key}.npz")
...
def fmt_rmse(mean_arr, std_arr, ensemble_spread_arr=None, r2_arr=None,
    ...
    `param_rmse_dict` (Joint* baselines only) is `{param_name: rmse}` between
    the baseline's final per-window parameter estimate and the window's true
    physical parameter, same schema as the ML pipeline's `param_rmse_s0/s1`.
    """
    dims = ("X", "Y", "Z")
    out = {
    ...
    if param_rmse_dict is not None:
        out["param_rmse"] = param_rmse_dict
    return out
...
def run_and_cache_baselines(datasets, device, batch_size=1, da_window_steps=None,
    ...
    if da_window_steps is None:
        ...
        N = da_window_steps
    cache_path = os.path.join(BASELINES_DIR, f"baselines{suffix}.json")
    ...
    traj_path = os.path.join(BASELINES_DIR, f"baselines_trajectories{suffix}.npz")
    all_present = all(
...
def evaluate_joint_baseline(method, dataset, cfg, device, return_trajs=False, batch_size=1,
                             use_corrupted_forcing=None, param_names=_PARAM_NAMES):
    """Like `evaluate_baseline`, but for the Joint* baselines that estimate
    physical parameters alongside the state.

    The baseline is fed the dataset's (possibly biased) sigma/rho/beta/c1 as
    its prior/regularization center (same as `evaluate_baseline`); its final
    per-window parameter estimate (last timestep of `BaselineResult.params`)
    is then compared against the window's true_sigma/true_rho/true_beta/
    true_c1 to get a param RMSE, mirroring the ML pipeline's `param_rmse`.
    """
    sig, rho, bet = cfg.da_params
    rmse_list = []
    results_list = []
    ...
    if batch_size > 1 and hasattr(method, 'assimilate_batch'):
        for i in range(0, len(dataset), batch_size):
            ...
            if has_per_window_params:
                sigma = torch.tensor([w["sigma"] for w in batch], device=device)
                rho_t = torch.tensor([w["rho"] for w in batch], device=device)
                beta = torch.tensor([w["beta"] for w in batch], device=device)
                c1 = torch.tensor([w.get("c1", 1.0) for w in batch], device=device)
            else:
                sigma, rho_t, beta = sig, rho, bet
                c1 = 1.0
            results = method.assimilate_batch(obs, mask, force, truth, sigma=sigma, rho=rho_t, beta=beta, c1=c1)
            for result_idx, result in enumerate(results):
                analysis = result.trajectory
                ref = truth[result_idx].detach().cpu().numpy()
                all_sq_err.append((analysis - ref) ** 2)
                ...
                rmse_list.append(result.rmse)
                results_list.append(result)
                param_est_list.append(result.params[-1])
                param_true_list.append(_true_params(batch[result_idx]))
    else:
        for i in range(len(dataset)):
            w = dataset[i]
            obs = w["obs"].to(device)
            ...
            results_list.append(result)
            param_est_list.append(result.params[-1])
            param_true_list.append(_true_params(w))
    ...
    all_ref = np.concatenate(all_ref, axis=0)
    pooled_mse = np.mean(all_sq_err, axis=0)
    pooled_var = np.maximum(np.var(all_ref, axis=0), 1e-12)
    expvar = 1 - pooled_mse / pooled_var
    expvar_stats = (expvar, np.zeros_like(expvar))

    pred_params = np.stack(param_est_list, axis=0)
    true_params = np.stack(param_true_list, axis=0)
    prmse = _param_rmse_fn(pred_params, true_params)

    if return_trajs:
        return (rmse_stats, expvar_stats, prmse), results_list
    return rmse_stats, expvar_stats, prmse


def run_and_cache_joint_baselines(datasets, device, batch_size=1, da_window_steps=None,
                                   weak_config=None, strong_config=None, enkf_config=None,
                                   etkf_config=None, suffix="", param_names=_PARAM_NAMES):
    """Same shape as `run_and_cache_baselines`, but for the Joint* (state +
    parameter) baselines. Reuses the exact same s0/s1 datasets/dynamics/case
    config, so results are directly comparable to the state-only baselines
    and to the joint ML models (`joint_cfm`/`joint_direct_unet`)."""
    if da_window_steps is None:
        N = int(3.0 / 0.01)
    else:
        N = da_window_steps
    cache_path = os.path.join(BASELINES_DIR, f"baselines_joint{suffix}.json")
    ...
    if os.path.exists(cache_path):
        ...
        print(f"  Running joint baselines (da_window_steps={N})...")
    ...
    for expo in {c[5] for c in _BASELINE_CASES}:
        dynamics = dynamics_pool[expo]
        baseline_pool[expo] = {
            "Joint-Weak-4DVar": JointWeak4DVar(dt=0.01, da_window_steps=N, device=device,
                                                coupling_exponent=expo, dynamics=dynamics, **weak_cfg),
            "Joint-Strong-4DVar": JointStrong4DVar(dt=0.01, da_window_steps=N, device=device,
                                                    coupling_exponent=expo, dynamics=dynamics, **strong_cfg),
            "Joint-EnKF": JointEnKF(dt=0.01, device=device, coupling_exponent=expo, dynamics=dynamics, **enkf_cfg),
            "Joint-ETKF": JointETKF(dt=0.01, device=device, coupling_exponent=expo, dynamics=dynamics, **etkf_cfg),
        }
    ...
    for case_name, ds_key, case_val, bias, label, coupling_exponent in _BASELINE_CASES:
        ...
        cfg = cfg_map[case_name]
        method_map = baseline_pool[coupling_exponent]
        for name in _JOINT_BASELINE_METHODS:
            if partial.get(case_name, {}).get(name) is not None:
                print(f"    {label}/{name:<18} already done, skipping")
                continue

            method = method_map[name]
            print(f"    {label}/{name:<18} ...", end=" ", flush=True)
            t1 = time.time()
            ((m, s), (r2, _), prmse), bl_results = evaluate_joint_baseline(
                method, ds, cfg, device, return_trajs=True,
                batch_size=batch_size, use_corrupted_forcing=(case_val == 2),
            ...
            param_rmse_dict = {nm: float(prmse[i]) for i, nm in enumerate(param_names)}

            if case_name not in partial:
                partial[case_name] = {}
            partial[case_name][name] = fmt_rmse(m, s, ensemble_spread, r2, crps_mean, crps_std, elapsed,
                                                 param_rmse_dict=param_rmse_dict)
            partial["total_time_seconds"] = time.time() - total_t0
            with open(cache_path, "w") as f:
                json.dump(partial, f, indent=2)
            ...
            if all_ens_var is not None:
                traj_data["ensemble_variance"] = all_ens_var
            np.savez_compressed(_baseline_traj_path(case_name, name, f"_joint{suffix}"), **traj_data)

            param_str = " ".join(f"{nm}={param_rmse_dict[nm]:.4f}" for nm in param_names)
            print(f"X={m[0]:.4f} Y={m[1]:.4f} Z={m[2]:.4f}"
                  f"  mean={np.mean(m):.4f}  param_rmse[{param_str}] [{elapsed:.1f}s]")

    # Combine per-method trajectory files into final .npz
    traj_path = os.path.join(BASELINES_DIR, f"baselines_joint_trajectories{suffix}.npz")
    all_present = all(
        os.path.exists(_baseline_traj_path(case_name, name, f"_joint{suffix}"))
        for case_name, _, _, _, _, _ in _BASELINE_CASES
        for name in _JOINT_BASELINE_METHODS
    )
    ...
    if all_present:
        ...
        for case_name, _, _, _, _, _ in _BASELINE_CASES:
            for name in _JOINT_BASELINE_METHODS:
                src = _baseline_traj_path(case_name, name, f"_joint{suffix}")
                data = np.load(src)
                prefix = f"{case_name}_{name.replace('-', '_').replace(' ', '_')}"
                for key in data.files:
                    traj_arrays[f"{prefix}_{key}"] = data[key]
