...
## 2026-09-01: QG1L structural-mismatch DA divergence — diagnosis + model-error-aware R lever

**Summary:** Diagnosed why the S1-QG1L DA (1-layer structural model error) is **worse than the free forecast** in q-space at nx=64 (EV −11, improv ~0.39). Verified the psi observation operator is **not** the bug: `_psi_h` correctly maps the 1-layer state to upper-layer ψ columns, and obs/r_var/localization are dimensionally consistent (the hardcoded `nlayers=2` at the `_build_qg_col_loc_matrices` call site is a dead parameter — the builder derives layer count from `state_dim`). The blowup is **filter divergence from unmodelled structural model error**: with a structurally-wrong model the nonlocal ψ obs are mutually inconsistent across timesteps, while `r_var` encodes only obs *noise* and the ensemble dispersion under-represents the O(1) model mismatch, so the ETKF over-trusts conflicting ψ obs and the spectral inversion drives the q-state off truth.

Added an **`obs_var_r_scale`** knob (default 1.0, backward-compatible) multiplying `r_var` on both q- and psi-obs paths (observation-covariance inflation that models error the perfect-forecast ensemble assumption understates). **Production (nx=64) result is honest: R-inflation moves QG1L psi-obs `forecast_improvement` monotonically toward the free forecast (rs 1 → 0.392, rs100 → 0.425, rs1e4 → 0.813, heading to 1.0 as R→∞) but does NOT cross 1.0 at production resolution.** The nx=16 "full restoration" (rs100 → improv 1.02) is a coarse/short-window artifact and does not transfer; q-obs is also NOT inherently robust at nx=64 (improv 0.99 vs the 1.29 seen at nx=16). The S0 (error-free) anchor is unaffected by the knob (EV ≊0.75, improv >1).

**Deferred (not shipped):** the existing `etkf_additive` is a raw absolute term (`randn * add`), and a probe showed even a regime-relative additive provably over-disperses the localized ETKF at nx=64, diverging even the *error-free* S0 filter (EV −86/−3798). A correct model-error additive must be applied to the forecast/background ensemble at a background-spread scale, not the collapsed posterior — deferred to a separate follow-up.

**Files modified:**
- `evaluation/run_qg_baselines.py` — `_make_obs_system`/`run()` gain `obs_var_r_scale` (default 1.0) applied to `r_var`; `--obs-var-r-scale` CLI.
- `evaluation/sweep_qg_baselines.py` — `--obs-var-r-scale-list` sweep knob (threaded into `run()` and the output tag).
- `tests/test_qg_baselines.py` — new `test_qg1l_psi_obs_r_scale_restores_da_over_free_forecast` (default psi-obs `improv<1`, inflated R shows monotone improvement toward the free forecast, S0 unaffected) + new `test_make_obs_system_r_scale_wiring` (pins `_make_obs_system` returns `r_var * scale` on both q- and psi-obs branches so a refactor cannot silently drop the multiplier).
- `tests/test_qg_random_columns.py` — updated stale `test_random_columns_daily_cadence_and_mask` → `test_random_columns_one_event_per_day_and_mask`: the obs timing became "random sub-daily obs" in #94 (one event/day at a random sub-daily offset), but the test still asserted the old fixed daily cadence `range(0, num_steps, spd)` and was failing; now asserts the actual invariant (one event per day, each strictly inside its day's window, observed `obs_columns >= 0`). Required so the full QG test suite is green before it enters the master CI gate.
- `batch/run_qg_s1_qg1l_rscale_probe.sbatch` — new: 4-task nx=64 array over {psi rs1, psi rs100, psi rs1e4, q rs1} on `test_s0,test_s1_qg1l`.
- `reports/qg_s1_qg1l_rscale_probe.py` — new: CPU probe driver (nx=16) used for the small-scale confirmation.
- `reports/outputs/qg_s1_qg1l_rscale/*.json` — production nx=64 probe results (4 R-scale JSONs, job 51316).
- `CHANGELOG.md` — this entry.

**Rationale:** The user's intuition that DA should not be *catastrophically* worse than the wrong-model free forecast was correct, but the mechanism is more subtle than the small-scale probe suggested: R-inflation is well-motivated yet necessary-but-**insufficient** at production resolution — it moves DA monotonically toward the free forecast but plateaus below 1.0. The honest, confirmed deliverable is the diagnosis plus the backward-compatible R lever; a working model-error additive is out of scope here.

**Verification:** `pytest tests/test_qg_dynamics.py tests/test_qg_data.py tests/test_qg_baselines.py tests/test_qg_s0s1.py tests/test_qg_random_columns.py tests/test_qg1l_dynamics.py -m "not slow"` — **99 passed** (full QG fast suite green; includes the fixed stale test + both new tests). `ruff check` clean on all touched code (only pre-existing `RUF059` at `test_qg_baselines.py:199` remains, unchanged). Production nx=64: psi rs1/rs100/rs1e4 improv 0.392/0.425/0.813, q rs1 0.992; S0 anchor ≊0.75 (unaffected by the knob).
...
**Files modified:**
- `evaluation/baselines.py` — `ETKF.assimilate` localized gain: ridge is now scale-relative `(1e-4 if etkf_ridge<=0 else etkf_ridge) * loc_H_Pf_Ht.max()` instead of absolute `1e-4`.
- `tests/test_qg_baselines.py` — new `test_localized_etkf_r_sensitive_analysis`: constructs a localized ETKF and asserts the analysis RMSE changes when `R_var` is lowered by 1e4 (guards the R-invariance bug; verified to fail on the old code and pass on the fix).
- `CHANGELOG.md` — this entry.

**Rationale:** Stage B2 (the post-PR-#118 low-noise sweep) still returned R-invariant results even after fixing the obs-noise wiring — proving the sub-1 improv was not a wiring/regime artifact but a filter bug: an absolute ridge that nullified the gain regardless of R or obs density. Making the ridge covariance-relative restores the Kalman filter's dependence on R and unlocks `forecast_improvement > 1`, which was the original Stage B goal (an obs config where DA beats the free forecast).

**Verification:** `test_localized_etkf_r_sensitive_analysis` passes with the fix and fails (identical RMSE, `0.0 > 1e-10`) on the old buggy ridge. QG suite 80/80 pass (`test_qg_baselines` 22, `test_qg_s0s1` 13, `test_qg_data` 13, `test_qg_dynamics` 22, `test_qg1l_dynamics` 16 — not slow). `test_baselines_hydra` 12 pass; the only `test_baselines_enkf` failures (2, `dynamics=None`) are pre-existing on base (verified via stash). ruff: no errors on the touched line (only the pre-existing `_obs_spec_rc` RUF059 in the test file remains). Empirical (nx=16 CPU, shared-init ETKF): improv before 0.62 → after, nz=0.05 lag1 0.83 / nz=0.001 lag1 **2.67**, lag2 2.73.

## 2026-08-29: QG obs-noise-propagate fix in run() + low-noise obs-enrichment sweep (Stage B2)

**Summary:** Found and fixed a real config-wiring bug in `evaluation/run_qg_baselines.py` that silently made the DA obs-noise independent of the requested `obs_noise_std_frac` — which is why the Stage B obs-enrichment sweep (job 50876) returned byte-identical `nz=0.02`/`nz=0.05` results. Root cause: `run()` **rebuilt** `cfg` from a fresh `QGConfig` in both the `ds is None` and `ds is not None` branches, dropping `obs_noise_std_frac` (and any other non-listed field, plus hardcoding `seed=7`). Because DA obs are regenerated inside `_make_obs_system` → `_q_alongtrack_obs` from `cfg.obs_noise_std_frac`, the noise sweep was pinned to the 0.05 default the whole time. Fix: `run()` now carries the passed `cfg` through unchanged (only builds a dataset from it when `ds is None`), eliminating the whole class of silent field-drop bugs. Diagnostic experiments also disproved the "degenerate ensemble" hypothesis: per-step ensemble spread stays healthy (~1e-8 relative units, no collapse) through the ETKF, and `disp_frac=0` reproduces the free forecast exactly (improv=1.0) while positive dispersion makes DA monotonically worse — the filter is alive, not degenerate. The sub-1 `forecast_improvement` at the EV≈0.7 reference lag is a regime property: at short lag the model-error-free free forecast is already more accurate than the observations (nx=16: bg error ~4e-9 vs obs noise 1.5e-8), and even where obs noise < background (nx=64: obs_std ~1.3e-6 vs ff_rmse 7.3e-6) DA still does not lift pooled EV above the free forecast because along-track coverage is sparse. Stage B2 re-tests with a genuinely low-noise range once the fix is in.

**Files modified:**
- `evaluation/run_qg_baselines.py` — `run()` no longer rebuilds `cfg`; passes its `cfg` argument through directly (only calls `make_qg_s0_s1_datasets(cfg)` when `ds is None`), so `obs_noise_std_frac` (and all other config fields) reach the obs generator.
- `tests/test_qg_baselines.py` — new `test_run_preserves_obs_noise_std_frac`: asserts `_q_alongtrack_obs` r_var scales with `obs_noise_std_frac` (low ≪ high) and that `run()` with a low-noise config completes and keeps the field.
- `batch/run_qg_ff_obs_sweep2.sbatch` — new: 12-task array (cols {4,8,16,32} × obs_noise_frac {0.05,0.01,0.001}) at lag 1.0, ETKF, N=80, infl 1.0, loc 6, S0, outdir `reports/outputs/qg_ff_obs_sweep2`. Submitted as job 50912.

**Rationale:** The unity-gate question ("is there an obs config where DA beats the free forecast") was being answered with silently-broken noise control. Fixing the propagation makes the noise sweep meaningful and lets us test the low-noise regime where observations are genuinely cleaner than the background — the only physically plausible route to `forecast_improvement > 1` under the perfect-model S0 setup. The ensemble-degeneracy hypothesis was explicitly ruled out by measuring live per-step spread.

**Verification:** `tests/test_qg_baselines.py` 21/21 pass (incl. new regression); QG suite `test_qg_baselines`+`test_qg_s0s1`+`test_qg_data`+`test_qg1l_dynamics` 62 passed / 4 deselected (not slow). `ruff check` clean on `run_qg_baselines.py` and on the new test code (only the pre-existing `_obs_spec_rc` RUF059 remains, unchanged from HEAD). Diagnostics (CPU, nx=16, shared-init ETKF): no ensemble collapse; DA_rmse grows with disp_frac (0.5→0.93x, 1.0→0.80x, 3.0→0.43x improv), confirming a live filter in an unhelpful-info regime. Stage B2 job 50912 submitted.
