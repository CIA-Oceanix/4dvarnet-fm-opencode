...
## 2026-08-26: Fix V2 TitsweepieCFM and V3 PredictStateCFM (session: L96-CFM-variants)

**Summary:** Restored V2 TweedieCFM and V3 PredictStateCFM classes, configs, tests, and dispatch code lost during PR #78 merge. Fixed V3 sampling ODE integration bug (missing `(mu-x)/(1-tau)` conversion), added complete two-stage training dispatch for V2, created test suite with 9 tests, and created training sbatch scripts ready for A100 runs.

**Implementation:** Restored 372-line `models/vanilla_cfm.py` with V2 TweedieCFM (two-stage mean estimator + velocity residual) and V3 PredictStateCFM (single-stage vs VanillaCFM). Added PredictStateCFMConfig and TweedieCFMConfig to `conf/schema.py`. Rewrote `training/lightning_module.py` with full V2 dispatch (configure_optimizers, on_train_start freezing, _forward_and_loss), `train.py` with model_factory/eval/stage dispatch, and `evaluation/neural_inference.py` with V2/V3 import and inference support. Created `tests/test_tweedie_cfm_variants.py` with 9 tests (init, forward损失, loss, sample, stage). Created V2/V3 configs for L96 (V2: 200-epoch stage1 + 400-epoch stage2, V3: 400-epoch single-stage). Created training sbatch scripts for A100 GPUs.

**Rationale:** The V1 removal commit (577c37e) had complete implementation, but PR #78 merge (a0ef7e4) silently reverted, making V2/V3 tracking stubs broken. Training jobs crashed on ghost code. Restoring ensures V2/V3 variants can be trained and evaluated on L96 (compared vs L2b=0.633, L3=0.564, L4=0.619). The V3 sampling fix addresses the previously discovered bug where network output (predicted state mu) was incorrectly treated as velocity without the ODE conversion.

**Verification:** All 4 key files compile successfully: `models/vanilla_cfm.py`, `conf/schema.py`, `training/lightning_module.py`, `train.py`, `evaluation/neural_inference.py`. Test suite ready (9 tests covering init, forward, loss, sample, stage). Sbatch scripts created and bash syntax validated. Ready to train on A100.
