...
**Verification:** `pytest tests/test_eval_da_params.py tests/test_param_head.py tests/test_l96_normalization.py` (29 passed, 3 skipped); re-run S0 identical to the pre-fix results (bit-for-bit, or to 3e-4 RMSE for the two older P1 runs); reports re-rendered with the consistency checks passing.
