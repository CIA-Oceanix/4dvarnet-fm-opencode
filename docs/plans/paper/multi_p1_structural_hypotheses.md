...
> into H3a (state representation) and H3b (autoregressive error propagation)**,
> which v1 conflated: §4.1.2's model-vs-posterior sensitivity argument was
> filed as a corollary of H3 when it is a hypothesis in its own right. A
> **fourth hypothesis, H4 (the value of gradients/adjoints)**, is named here and
> **owned by P2**, with only its classical corollary (C7) retained in this
> paper. No claim in v1 was retracted; C2/C3/C6 are re-tagged, C7 is new.
...
Data assimilation rests on a handful of commitments so standard they are rarely
written down as hypotheses:

- **H1 (Markovianity).** The joint inference problem over a window — or over a
  whole reanalysis — is replaced by a *sequence of simpler problems*, each
  conditionally independent of the past given the current state. This is what
  licenses cycling: propagate, update when observations arrive, repeat, each
  cycle re-initialized from the last. Filtering takes observations up to `t`;
  4D-Var and smoothers widen the unit to one window but keep the recursion
  between windows.
...
- **H3a (ODE/PDE state representation).** The state is a point in a phase space
...
- **H3b (autoregressive error propagation).** Because that prior is generated by
  *integrating* the model, every prior error is carried forward by the same
  recursion that carries the signal, and compounds along the window at the
  system's Lyapunov rate. Prior information reaches the analysis by one route
  only: through the integration.

**Why H3 is split.** v1 stated H3a alone and filed H3b's content as a corollary
(§4.1.2). They are separable, and they fail differently: H3a is about *which
variable you call the state* and is falsified by re-parameterization at
identical physics (§4.3); H3b is about *how error travels* and is falsified by
showing that the model's sensitivity to a perturbation is not the posterior's
sensitivity to it (§4.1.2). D4 tests the first, D7 the second.
...
the model-error axis that P1 owns. See §10 decision 6.

Each buys something real. The thesis of this paper is that **these jointly
...
|---|---|---|---|---|
| **H1** | the joint problem factorizes into a sequence of conditionally-independent simpler ones | O(1) memory, operational streaming, no training set | cannot adapt to the *observation configuration*; no amortization across windows | window-amortized estimators; smoothers |
...
| **H3a** | state = point in ODE phase space; uncertainty = perturbation | tractable propagation; physical interpretability | the *choice of state variable* fixes the conditioning of the inverse problem; Gaussian update ⇒ `Ψ_NG ≡ 0` | alternative state variables; non-Gaussian / generative posterior representations |
| **H3b** | prior errors are propagated by the same integration that propagates the signal | one mechanism for signal and uncertainty alike; no separate error model needed | error compounds at the Lyapunov rate, and *model* sensitivity is inherited by the analysis whether or not the *posterior* has it | direct (non-integrating) posterior parameterizations; amortized conditioning on `(θ, z)` |
| **H4** *(named, owned by P2)* | gradients/adjoints of the model or its variational cost are what make an estimator efficient | principled descent directions; the entire variational tradition | the adjoint must exist, be stable, and be evaluated at inference | closed-form or proxy gradients; amortizing the adjoint into learned weights |

**H4's row is stated for completeness and argued in P2.** This paper cashes in
only C7 (§4.2.1), the classical corollary that sits on the model-error axis.
...
### 4.1.2 H3b: ODE sensitivity is not a proxy for posterior sensitivity
...
and its apparent force is partly an artifact of **H3b** -- the autoregressive
propagation that routes all prior information through the integration. H2 and
H3b are not independent hypotheses; **H3b is what makes H2 bind**, and that is
precisely why v2 promotes it out of a corollary and into a hypothesis of its
own.
...
### 4.2.1 C7: the adjoint's value is conditional on model fidelity

The same table read down a different column. Strong-4DVar is the only classical
baseline here that uses a **gradient of the dynamics** (autodiff minimization
over the window); EnKF/ETKF are gradient-free. All three numbers below are from
the *one* config of §4.2, so no cross-quoting is involved (R3):

| | gain from 2× obs, S0 | gain from 2× obs, S1 | collapse |
|---|---|---|---|
| Strong-4DVar (**adjoint**) | 19.7% | **3.2%** | **6.2×** |
| ETKF (gradient-free) | 19.7% | 10.3% | 1.9× |
| EnKF (gradient-free) | 17.2% | 9.0% | 1.9× |

At S0 the adjoint method and the best ensemble filter convert extra
observations into skill at an **identical** rate (19.7% each). Under model
error the adjoint method's return collapses **3.3× harder** than either
gradient-free filter's. The gradient is not neutral under model error: it is
the mechanism by which a *wrong* model is trusted most thoroughly, because the
descent direction is computed through the very dynamics that are wrong.

**State this as a claim about marginal value, not about levels.** Strong-4DVar
remains the best classical method at every density in both scenarios of this
config (e.g. S1 Obs200 1.4751 vs ETKF 1.6367), so "the adjoint stops helping"
would be false. What collapses is `dSkill/d(density)`, which is exactly the
quantity S-a is about. *(§4.1's config gives a smaller S0 edge — 0.8116 vs ETKF
0.8883, i.e. 8.6% — narrowing to 2.5% at S1, 1.4617 vs 1.4998; but that is a
different config and per R3 must not be quoted in the same breath as the table
above.)*

**This is as far as P1 takes H4.** The constructive half — that a solver can
keep the differentiable prior for *training* and drop the gradient evaluation
at *inference* — is P2's, and the evidence for it is the L96 `obs+state` /
`subgrad+state` / `grad+state` / `gradsplit+state` ladder, not the DA
baselines. See §10 decision 6.

### 4.3 H3a: the *choice of state variable* changes DA skill, at identical physics
...
**This is H3a in one experiment.** Identical dynamics, identical information, and
...
### 4.4 H1 + H3a: the ensemble-Kalman class carries `S(Ψ_NG, 0)` in full
...
### 4.5 Localization and inflation as H3a symptoms, with the sensitivity work done
...
at finite `N` — an H3a cost — and the fact that its optimum tracks the
...
- **C2 (H3a).** The ODE state representation fixes the conditioning of the inverse
...
- **C3 (H1).** Markovian factorization — processing the problem as a cycled
  sequence of simpler ones — forecloses adaptation to the
...
- **C4 (H1+H3a ⇒ S-b).** The ensemble-Kalman class carries `S(Ψ_NG, 0)` in full by
...
- **C6 (H2×H3b).** The autoregressive representation's sensitivity to
...
- **C7 (H4, classical corollary).** The value of the adjoint is **conditional on
  model fidelity**. At S0 Strong-4DVar and ETKF convert extra observations into
  skill at the same rate (19.7% each per doubling); under model error the
  adjoint method's return collapses 6.2× against the gradient-free filters'
  1.9×. The gradient is the mechanism by which a wrong model is trusted most
  thoroughly. *The constructive half of H4 is P2's; see §10 decision 6.*
...
strong-constraint strawman. L63 and QG both have it and it is best-or-near-best
under model error. **Still the highest-value, lowest-cost item in this
document** — but its status improved substantially between v1 and v2 and the
remaining work is now narrow.

*Status as of 2026-09-20.* v1 recorded only that `Weak4DVar` exists in
`evaluation/baselines.py` with `--skip-weak` set in essentially every L96
sbatch. Since then, **open PR #202** (`feature/l96-fdv2-stier-sweep`) adds a
dedicated **`L96Weak4DVar`** class — QG4DVar's whitened background/model-error
controls, gradient sanitization and NaN-reset, ported rather than patched into
the shared `Weak4DVar` — plus 14 unit tests. That work also found *why* the
plain class failed on L96: **Adam, not the weak-constraint parameterization.**
Through a 500-step chaotic RK4 unroll, Adam (150 steps, lr=0.02) leaves
`Jo ≈ 80–90k` against an expected noise floor of 60–100; LBFGS with
`Strong4DVar`'s own validated settings (`max_iter=10, lr=0.2`) collapses `Jo` to
77–195 and is 6–10× faster per sub-window.

**What is left is one benchmark run, not an investigation.** No S0/S1 run
exists yet — only unit tests and ad hoc diagnostics. Two cautions carried over
from that PR's own fragments: (a) the ~1.52 RMSE reported mid-investigation is
**retracted**
by a same-day follow-up as having been measured on the wrong seed with 3
windows; the corrected `seed=123`/10-window figure is **~0.98**, close to
`Strong4DVar` — do not cite ~1.52; (b) the class's `optimizer` default is still
`"adam"` and flipping it is an explicit decision, not a silent one.
...
**D4 — H3a by re-parameterization.** The QG ψ/q result generalized: assimilate the
...
companion doc's 93%/7% split. The single highest-value item here — and, as of
v2, mostly *unblocked*: the `L96Weak4DVar` class, the LBFGS fix and the tests
all exist in open PR #202. The P0 is now "land #202 (currently `DIRTY` — it
needs a rebase) and run the S0/S1 benchmark", not "implement weak-4D-Var".
...
5. Reporting convention for S-b: adopt the DA literature's rank
   ...
   those.
6. **H4's split between P1 and P2 — proposed in v2, not yet ratified in P2.**
   This doc now names H4 and keeps only C7 (§4.2.1), the classical corollary on
   the model-error axis. The constructive claim — *differentiability is a
   training-time requirement, not an inference-time one* — is assigned to P2
   under the §9 boundary (operator family, S0). **`p2_unrolled_solvers_and_flows.md`
   has not been updated to accept it**, so until it is, H4 is named in one doc
   and argued in neither. Resolve before either paper is drafted.

   *What the evidence looks like today, for whoever picks this up.* On master the
   L96 S-tier ladder has two rungs (pooled S0 RMSE): `FDV1-Stier` `obs+state`
   (no cost function) **0.4012**, `subgrad+state-Stier` (closed-form gradient of
   the same cost — `obs−x` plus the denoising residual `x−Φ(x)`, no
   `autograd.grad` at inference) **0.3729**. Open PR #202 adds the third:
   `grad+state-Stier-priorresidual-tbptt2x5` (true `torch.autograd.grad` of the
   summed `var_cost`) **0.4098**, and a `gradsplit+state` mode — true per-term
   gradients fed as *separate* channels — which is the control that separates
   "closed form beats autodiff" from "unmixed beats summed", implemented and
   tested but **not yet run**. Three caveats before any of this is claimed: the
   `grad+state` checkpoint had ~140 of 400 epochs left and had not plateaued;
   it also carries two fixes (`prior_residual=true`, `prior_dropout=0.0`) that
   `subgrad+state` does not, so it was *helped* and still lost; and every rung
   is a single seed.
