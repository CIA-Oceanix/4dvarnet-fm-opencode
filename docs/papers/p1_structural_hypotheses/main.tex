...
%
%  SOURCE OF TRUTH: docs/plans/paper/multi_p1_structural_hypotheses.md (SCOPING v2).
...
strong-constraint 4D-Var a 3.2\% RMSE reduction against 20.6\% in the
perfect-model case, a $6.4\times$ collapse in the marginal value of data, while
gradient-free ensemble filters keep most of it (a factor of $1.1$).
%
...
variable produces large and predictable skill differences; and that learned
estimators are robust to corrupted model information only when trained to expect
it. The ensemble-Kalman class zeroes the non-Gaussian component of the posterior
by construction, but neither dispersion nor rank histograms show this as a
deficit specific to it.
%
