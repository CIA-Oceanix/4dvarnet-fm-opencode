...
\begin{description}[leftmargin=2.2em,style=nextline]
  ...
  \item[\textbf{C4 (H1 $+$ H3a $\Rightarrow$ S-b) --- \emph{not supported as
    ...
    \textbf{The dispersion measurements refute the obvious form of this}
    (Section~\ref{sec:nongaussian}): ETKF and EnKF are the best-calibrated
    schemes measured (spread/RMSE $1.127$, $1.089$ against a target of $0.967$),
    while every scheme with a full non-Gaussian branch is under-dispersed by a
    factor of two or more. The reconciliation is that spread/RMSE is a
    second-moment diagnostic and $\PsiNG$ is not a second-moment object --- a
    Gaussian update can match the spread exactly and still be wrong about the
    posterior's shape. That is a coherent position but an untested one, because
    the diagnostic that would test it (rank histograms) does not exist in this
    codebase. \textbf{C4 is therefore downgraded to an open question}, and S-b
    rests for now on the accuracy/dispersion trade being unresolved by
    \emph{every} class rather than on a deficit specific to one.
