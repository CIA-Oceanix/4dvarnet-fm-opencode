...
| `docs/plans/analysis/qg_specwind_da_s0.md` | **DRAFT** v1 2026-09-26. ETKF/EnKF S0 on the forced and coupled spectral-wind QG test sets, ocean state only, with the true wind prescribed (a perfect model for both). Nadir-like ψ₁ observations: 3 random meridional columns per day (4.7%), each once per day. Tuning on val, main runs on test, a density curve, and factor-stratified skill. About 21 GPU-hours; the data sit on a server-local disk (decision 1). Nothing run. |
