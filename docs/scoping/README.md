...
| `cfm_tau_consistency_next_steps.md` | **DRAFT**, v3 2026-09-23. Plan derived from the tower property, with an execution plan. Three parallel tracks: diagnostics (NS1 truth-marginal test, NS0), training code (EMA teacher plus one shared pair construction), and a T0 reseed. Batch 1 (T0, T1′ control, T2a bootstrapped τ=0 targets) attacks the biased τ=0 prediction; Batch 2 (T2b, T1), gated on NS1, attacks the variance decay; T4a and T3 are fallbacks. Measured defects in `docs/results/cfm_tau_consistency.md`. Nothing implemented; decisions in §7. |
