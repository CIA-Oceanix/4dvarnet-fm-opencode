...
| `cfm_tau_consistency_next_steps.md` | **DRAFT**, v4 2026-09-24. Targets defect 2 (variance collapse). S1 is done: early-fine Euler steps are adopted as the sampler default (#253). T5 is done and negative: the single-probe objective is biased toward shrinking the Jacobian, and λ = 100 diverged. A corrected T5 is specified but not launched. Re-probed on the benchmark-default (L96B) flows at 400/1200 epochs (`docs/results/cfm_tau_consistency_l96b.md`): the τ=0 bias persists and grows relative to the endpoint; the variance collapse is mostly sampler-side; early-fine N=20 is the best sampler. |
| `cgoa_coupled_gyrostat_case_study.md` | **SCOPING** v1 2026-09-24. A fourth case study: coupled ocean–atmosphere from coupled gyrostats (Tier A), plus MAOOAM through qgs (Tier B), on one quadratic-tensor torch engine. Nothing implemented. Paper fit (P1 / P2 / standalone) is open, §9. |
