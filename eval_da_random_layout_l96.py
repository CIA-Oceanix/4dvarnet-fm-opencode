...
channels keep the P1 initialization. Everything else is the P1 DA path
...
def draw_layout(truth_obs: torch.Tensor, n_obs_range, fast_range, seed: int) -> tuple[torch.Tensor, torch.Tensor]:
    """(T, od) obs and (T,) mask for one window's observed-space truth."""
...
def build_cell(datasets: dict, windows: list[int], n_draws: int, n_obs_range, fast_range,
               obs_var_indices) -> tuple[dict, dict]:
        ...
        for wi in windows:
            ...
            for d in range(n_draws):
                obs, mask = draw_layout(w["true_state"][:, idx].float(), n_obs_range, fast_range,
                                        _layout_seed(case, wi, n_obs_range, fast_range, d))
...
def _traj_path(tag: str, inflation: float) -> str:
    inf = f"_inf{inflation}_etkf_inf{inflation}" if inflation != 1.0 else ""
    return os.path.join(EXP_DIR, f"l96_baselines_trajectories_dws500{tag}{inf}_obsj2_fw_dafw.npz")
...
def main() -> None:
    ...
    z = np.load(_traj_path(tag, args.inflation))
    ...
    summary = {"n_obs_range": [lo, hi], "fast_range": [flo, fhi], "windows": [int(w) for w in windows],
               ...
               "source": os.path.basename(_traj_path(tag, args.inflation)), "cases": {}}
    arrays = {}
    for case, key in CASES.items():
